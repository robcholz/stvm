...
int SetTrieKeyVal(
    ...
    int key_size,
    void*
        val);  // 设置键-值，成功返回1，失败返回0，可以用此接口创建键-值和更改已有键-值
int DelTrieKeyVal(STRIE* trie,
                  unsigned char* key,
                  int key_size);  // 删除键-值，成功返回1，失败返回0
void* GetTrieKeyVal(
    STRIE* trie,
    unsigned char* key,
    int key_size);  // 根据键获得获得值，成功返回值，失败返回NULL，
// 返回NULL时不排除数据域的指针本来就等于NULL，
// 需配合TrieExistKeyVal()接口判断
int TrieExistKeyVal(STRIE* trie,
                    unsigned char* key,
                    int key_size);  // 键-值是否存在，存在则返回1，不存在则返回0
...
int TrieTraverse(STRIE* trie,
                 TRIE_VISIT visit);  // 对树中所有元素依次调用visit函数
